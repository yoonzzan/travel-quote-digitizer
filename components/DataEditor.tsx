...
const DataEditor: React.FC<DataEditorProps> = ({ data, onChange }) => {
  ...
  const handleCostChange = (field: string, value: string | number) => {
    onChange({
      ...data,
  ...
  const calculatePerPersonKRW = () => {
    ...
    const pax = data.cost.internal_pax || 1;
    return Math.round(totalKRW / pax);
  };
  ...
  const handleAddDay = () => {
    ...
    onChange({ ...data, itinerary: [...data.itinerary, newDay] });
  };
                   ...
                   {uniqueCurrencies.map(curr => (
                      <div key={curr} className="flex items-center bg-slate-50 px-3 py-2 rounded-lg border border-slate-200 gap-3 shadow-sm">
                         <div className="flex items-center gap-2">
                            <span className="text-sm font-bold text-slate-700 bg-white border border-slate-200 px-2 py-1 rounded shadow-sm">
                               {curr} 1
                            </span>
            ...
            <div className="flex flex-col md:flex-row items-start md:items-end justify-between mt-2 gap-4">
               <div>
                   <h5 className="text-sm font-bold text-slate-700 flex items-center gap-1.5 mb-1">
                     <Calculator className="w-4 h-4 text-slate-400" /> 내부 정산용 상세 원가 (고객 노출 X)
                   </h5>
                   <p className="text-[10px] text-slate-400">항목별 상세 금액을 입력하여 원가를 계산합니다.</p>
               </div>
               ...
               <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto bg-white p-4 rounded-xl border border-slate-200 shadow-sm items-center">
                  ...
                  {/* Travel Pax Input - Improved UI */}
                  <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-start bg-slate-50 p-2 rounded-lg border border-slate-100">
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center bg-white rounded-md px-2 border border-slate-200 shadow-sm">
                        <input 
                          type="number"
                          value={data.cost.internal_pax || 1}
                          min="1"
                          onChange={(e) => handleCostChange('internal_pax', parseInt(e.target.value) || 1)}
                          className="w-16 py-1.5 text-center font-bold text-lg text-slate-900 outline-none bg-transparent"
                        />
                        <span className="text-xs text-slate-400 font-medium pr-2">명</span>
                    </div>
                  </div>
                  
                  <div className="h-px w-full sm:h-8 sm:w-px bg-slate-200"></div>
                  ...
                  <div className="flex flex-col items-end w-full sm:w-auto">
                     ...
                     {uniqueCurrencies.length > 0 && (
                        <div className="bg-hana-light/30 px-3 py-1.5 rounded-lg border border-hana-purple/10">
                            <span className="text-xs text-hana-purple font-medium mr-2">1인당 원가 (KRW 환산)</span>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
            ...
            <span className="text-xs font-medium text-slate-500 bg-white px-2 py-1 rounded border border-slate-200">
                {data.trip_summary.period_text || '기간 미정'}
            </span>
        </div>
                        ...
                        <div className="md:w-1/3 flex flex-col gap-4">
                             ...
                             <div>
                                ...
                                <div className="flex rounded-lg border border-slate-200 overflow-hidden">
                                    <div className="flex-1 flex flex-col border-r border-slate-200 bg-slate-50 group focus-within:ring-2 focus-within:ring-hana-mint focus-within:z-10 relative">
                                        <span className="px-2 py-1 text-[10px] text-slate-400 font-bold border-b border-slate-100 block">조식</span>
                                        <input 
                                            ...
                                            className="w-full text-xs p-2 bg-white outline-none text-center transition-all" 
                                        />
                                    </div>
                                    <div className="flex-1 flex flex-col border-r border-slate-200 bg-slate-50 group focus-within:ring-2 focus-within:ring-hana-mint focus-within:z-10 relative">
                                        <span className="px-2 py-1 text-[10px] text-slate-400 font-bold border-b border-slate-100 block">중식</span>
                                        <input 
                                    ...
                                    <div className="flex-1 flex flex-col bg-slate-50 group focus-within:ring-2 focus-within:ring-hana-mint focus-within:z-10 relative">
                                        <span className="px-2 py-1 text-[10px] text-slate-400 font-bold border-b border-slate-100 block">석식</span>
                                        <input 
