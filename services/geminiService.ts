...
export const parseSpreadsheet = async (file: File): Promise<string> => {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(new Error("파일 읽기(FileReader) 중 오류가 발생했습니다."));
    };
...
export const extractDataFromDocument = async (file: File, apiKey: string): Promise<TravelQuoteData> => {
  ...
  const systemInstruction = `
    ...
    - **Currency Awareness**:
      ...
      - If a column header says "Amount (USD)", currency="USD" for all rows.
      - Do NOT default currency to "KRW" if it is missing. Leave it empty if unsure.
    - **Number Extraction**:
